...
namespace LogReg
{
    public partial class CURD : Form
    {
        ...
        SqlConnection con = new SqlConnection(@"Data Source=SAZZAD\SQLEXPRESS;Initial Catalog=LogReg;Integrated Security=True;");
        public int InventorySl;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (IsValid())
            {
                SqlCommand cmd = new SqlCommand("insert into InventoryTbl values (@id, @name, @unitAvailable, @unitPrice)", con);
        ...
        {
            InventorySl = Convert.ToInt32(InventoryRecordDataGridView.SelectedRows[0].Cells[0].Value);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (InventorySl > 0)
            {
                SqlCommand cmd = new SqlCommand("update InventoryTbl set ProductId = @id, ProductName = @name, UnitAvailable = @unitAvailable, UnitPrice = @unitPrice where InventorySl = @Sl", con);
                cmd.CommandType = CommandType.Text;
                cmd.Parameters.AddWithValue("@id", p_id.Text);
                cmd.Parameters.AddWithValue("@name", p_name.Text);
                cmd.Parameters.AddWithValue("@unitAvailable", u_available.Text);
                cmd.Parameters.AddWithValue("@unitPrice", u_price.Text);
                cmd.Parameters.AddWithValue("@Sl", this.InventorySl);
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            if (InventorySl > 0)
            {
                SqlCommand cmd = new SqlCommand("delete from InventoryTbl where InventorySl = @Sl", con);
